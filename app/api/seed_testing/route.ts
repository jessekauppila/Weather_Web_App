...
// http://localhost:3000/api/seed_testing
...
const seedStations = async () => {
  ...
  if (stations && stations.length > 0) {
    for (const station of stations) {
      await client.sql`
        INSERT INTO stationsTest (id, stid, station_name, longitude, latitude, elevation, source)
...
const seedObservations = async () => {
  await client.sql`
    CREATE TABLE IF NOT EXISTS observationsTest (
      ...
      FOREIGN KEY (station_id) REFERENCES stationsTest(id),
  ...
  if (observations && observations.length > 0) {
    for (const obs of observations) {
      ...
      const query = `
        INSERT INTO observationsTest (${columnString})
